...
def generate_launch_description():
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(amr2ax_nav_dir, 'config', 'amrtcm.yaml'),
    ...
    localization_cmd_group = GroupAction([
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(amr2ax_launch_dir, 'include', 'amcl.launch.py')),
            condition=IfCondition(
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(amr2ax_launch_dir, 'include', 'gps_localization.launch.py')),
            condition=IfCondition(PythonExpression(["'", localization_type, "'=='GPS'"])),
            launch_arguments={'use_sim_time': use_sim_time,
                              'params_file': os.path.join(
                                  amr2ax_nav_dir, 'config', 'gps_localization.yaml')
                              }.items()),

        # 3D SLAM
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(amr2ax_launch_dir, 'include', '3d_localization.launch.py')),
            condition=IfCondition(
                PythonExpression([slam, " and '", localization_type, "'=='3D'"])),
            launch_arguments={'use_sim_time': use_sim_time,
                              'params_file':
                                  os.path.join(amr2ax_nav_dir, 'config', '3d_localization.yaml'),
                              'slam': 'True'}.items()),
