...
class ExperimentalTester(Node):
    ...
    def ensure_localization(self, expected_node_id=0):
        """Aggressive AMCL initialization: reinit + convergence spin (relative)."""
        self.reinitialize_amcl(expected_node_id)
        time.sleep(1.0)

        # Spin relativ mic pentru convergenta AMCL (nu afecteaza heading final)
        print('    [AMCL convergence spin]')
        self.exec_spin(math.pi / 3, timeout=10.0)
        time.sleep(1.0)
        ...
        if d > 1.5:
            ...
            self.reinitialize_amcl(expected_node_id)
            self.exec_spin(math.pi, timeout=15.0)
            time.sleep(1.5)
            for _ in range(10):
                rclpy.spin_once(self, timeout_sec=0.2)
            fp = self.odom.get_map_pose()
    ...
    def _prescan(self):
        """360deg scan: rotate to let semantic_localizer see all objects."""
