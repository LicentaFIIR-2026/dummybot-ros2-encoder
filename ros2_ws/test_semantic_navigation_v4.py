...
class ExperimentalTester(Node):
    ...
    def exec_follow_with_replan(self, goal_pose, timeout=180.0):
        ...
        3. Every REPLAN_CHECK_INTERVAL seconds:
           a. Check if robot is already near goal → skip replan
           b. Reload penalties from disk
           c. If penalties changed significantly on remaining edges → cancel + replan
        4. Repeat until goal reached or timeout
        5. If ComputeRoute fails → fallback to NavigateToPose
        ...
        while time.time() - t_global < timeout:
            ...
            if path is None or not path.poses:
                ...
                # Fallback: direct NavigateToPose (SmacPlanner, no Route Server)
            ...
            while not result_future.done():
                ...
                if time.time() - last_check >= REPLAN_CHECK_INTERVAL:
                    ...
                    # Skip replan if robot is close to goal
                    ...
                    if rp:
                        ...
                        if d2g < NODE_PROXIMITY_THRESH * 2:
                            continue  # almost there, don't replan

                    # Check if penalties changed significantly on route edges
                    new_pen = load_nav2_penalties()
                    changed = False
                    for eid_s, old_val in ep.items():
                        new_val = new_pen.get(eid_s, old_val)
                        if abs(new_val - old_val) > 2.0:  # was 0.5, too sensitive
                            changed = True
                            break
