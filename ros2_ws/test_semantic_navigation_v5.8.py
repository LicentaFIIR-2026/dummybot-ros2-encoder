...
Changes from v4 (heading fix):
  ...
  - TARGET_YAW_NODE0 / TARGET_YAW_NODE7 constants for required headings
...
GRAPH_NODES = {
    0: {'name': 'start',    'x': 1.0, 'y':  0.0},
    2: {'name': 'center',   'x': 3.4, 'y':  0.0},
    3: {'name': 'in_top',   'x': 2.4, 'y':  1.0},
    4: {'name': 'in_bot',   'x': 2.4, 'y': -0.5},
    5: {'name': 'out_top',  'x': 4.8, 'y':  1.0},
    6: {'name': 'out_bot',  'x': 4.8, 'y': -0.5},
    7: {'name': 'goal',     'x': 6.7, 'y':  0.0},
}
...
# Node 7 (goal): looking back toward start
#   quaternion z=-0.9995870503545529, w=0.028735496576264175
TARGET_YAW_NODE7 = 2.0 * math.atan2(-0.9995870503545529, 0.028735496576264175)
...
def identify_route(path_poses):
    if not path_poses:
        return 'UNKNOWN', {}
    key = {
        'in_top':  GRAPH_NODES[3],
        'in_bot':  GRAPH_NODES[4],
        'center':  GRAPH_NODES[2],
        'out_top': GRAPH_NODES[5],
        'out_bot': GRAPH_NODES[6],
    }
    ...
    top = 'in_top' in vis or 'out_top' in vis
    bot = 'in_bot' in vis or 'out_bot' in vis
...
class ExperimentalTester(Node):
    ...
    def ensure_localization(self, expected_node_id=0):
        ...
        if converged:
            target = TARGET_YAW_NODE0 if expected_node_id == 0 else TARGET_YAW_NODE7
    ...
    def run_setup(self, wait_secs=60):
        ...
        cfg = {
            ...
            'target_headings': {
                ...
                'node7_yaw_rad': TARGET_YAW_NODE7,
            },
        }
    ...
    def run_test1(self):
        ...
        t0 = time.time()
        path, result = self.compute_route(node_pose(0), node_pose(7))
        dt = time.time() - t0
    ...
    def _run_baseline_leg(self, start_id, goal_id, run_num, direction):
        ...
        traj_cls = [(p[0], p[1], '') for p in traj_for_dist]
    ...
    def run_test2(self, n_runs=5):
        ...
        for run in range(1, n_runs + 1):
            ...
            print('  [Pre-forward AMCL reinit at node 0]')
            self.ensure_localization(0)

            print('  [Forward] start(0) -> goal(7)')
            fwd_data, fwd_ok = self._run_baseline_leg(0, 7, run, 'forward')
            if fwd_data:
                self._save(f'test2_run{run:02d}_forward.json', fwd_data)

            print('    [Spin -> node 7 heading]')
            s_ok, s_dt, s_delta = self.spin_to_heading(TARGET_YAW_NODE7)
    ...
    def run_test3(self, n_runs=5):
        ...
        for run in range(1, n_runs + 1):
            ...
            print('  [Forward adaptive] start(0) -> goal(7)')
            fwd_data, fwd_ok = self._run_adaptive_leg(0, 7, run, 'forward')
            if fwd_data:
            ...
            print('  [Return adaptive] goal(7) -> start(0)')
...
def main():
    ...
    print(f'  Target yaw node7: {math.degrees(TARGET_YAW_NODE7):.2f}deg')
