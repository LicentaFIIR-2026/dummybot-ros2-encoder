...
DEFAULT_PENALTIES = {
    'dynamic': 10.0,     # person → high penalty, prefer alternate route
    'static': 3.0,       # chair → moderate penalty
    'minor': 0.5,        # bottle → low penalty
}
...
class SemanticMapManager:
    ...
    def _load_objects(self):
        ...
        try:
            ...
            for feat in data.get('features', []):
                ...
                # Handle both old format (obj_id in props) and new format (name in props)
                if 'obj_id' in props:
                    obj_id = props.get('obj_id', '')
                    metadata = props
                elif 'name' in props and 'metadata' in props:
                    obj_id = props.get('name', '')
                ...
                obj = PersistentObject(
                    ...
                    mobility=metadata.get('mobility', 'static'),
                    x=coords[0],
                    y=coords[1],
                    ...
                    ttl_seconds=metadata.get('ttl_seconds', -1.0),
                )
    ...
    def save_objects(self) -> bool:
        """
        Save persistent objects merged into the route graph as additional nodes.
        Creates a complete graph that can be opened in LIF editor.
        """
        ...
        # Start with the base route graph or create minimal structure
        if self.route_graph is not None:
            merged_graph = copy.deepcopy(self.route_graph)
        else:
            merged_graph = {
                'type': 'FeatureCollection',
                'name': 'semantic_objects_graph',
                'crs': {
                    'type': 'name',
                    'properties': {
        ...
        # Find max node ID from existing graph
        max_node_id = 0
        for feat in merged_graph.get('features', []):
            if feat.get('geometry', {}).get('type') == 'Point':
                node_id = feat.get('properties', {}).get('id', 0)
                max_node_id = max(max_node_id, node_id)

        # Add semantic objects as new nodes
        ...
        for obj in self.objects.values():
            ...
            merged_graph['features'].append(node_feature)
            next_id += 1

        # Update metadata
        merged_graph['date_generated'] = datetime.now(timezone.utc).strftime('%Y-%m-%d')
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f'Saved {len(self.objects)} semantic objects merged into graph '
                    f'at {self.objects_filepath}'
                )
            return True
        except Exception as e:
            if self.logger:
    ...
    def update_object(
        ...
        Returns the obj_id of the matched/created object.
    ...
    def annotate_route_graph(self) -> Optional[Dict]:
        ...
        for feature in annotated.get('features', []):
            ...
            total_penalty = metadata.get('penalty', 0.0)
            min_speed = metadata.get('speed_limit', 100)
